...
class MainScreen(Screen[None]):
    ...
    def _populate_rules(self) -> None:
        ...
        rows = [
            (rule.name, rule.id, rule.created_at[:10])
            for rule in self.store.load()
        ]
        ...
        for path in sorted(self.rules_dir.glob("*.yaml")):
            try:
                ...
                continue
            created = dt.datetime.fromtimestamp(path.stat().st_mtime).strftime(
                "%Y-%m-%d"
            )
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        try:
            ...
            is_successful = await asyncio.to_thread(
                create_and_save_rule, query, rule_id
            )
